...
const MyResponsiveChoroplethInter = () => {
  ...
  useEffect(() => {
    if (geo.current) {
      setProjectionScale(
        (geo.current.clientWidth * geo.current.clientHeight) / 5000
      );
  ...
  return features && real ? (
    <div style={{ height: "100%", width: "100%" }} ref={geo}>
      ...
      {popupTogel && (
        <div className="popup">
          <button className="close-btn" onClick={() => {setPopupTogel(false);}}>Close</button>
          <PoplineGraph locadionId={popupContent}/>
        </div>
      )}
    </div>
  ) : (
